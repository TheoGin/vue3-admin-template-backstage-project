...
const initAddSkuData = async (c1Id: string, c2Id: string, spu: SPUData) => {
  ...
  // 收集平台属性列表
  const resAttrList: AttrResponceData = await reqAttrByCategoryId(
    c1Id,
    c2Id,
    spu.category3Id,
  )
...
const saveAddSku = async () => {
  ...
  /* 注意是从attrList.value中遍历，而不是skuFormData.skuAttrValueList。
    ...
    初始值：[]
  */
  skuFormData.skuAttrValueList = attrList.value.reduce(
    (prev: any, next: any) => {
  ...
  /* 注意是从spuSaleAttrList.value中遍历，而不是skuFormData.skuSaleAttrValueList。
    ...
    初始值：[]
  */
  skuFormData.skuSaleAttrValueList = spuSaleAttrList.value.reduce(
    (prev: any, next: any) => {
