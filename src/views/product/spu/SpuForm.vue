    ...
    <el-form label-width="100px">
      ...
      <el-form-item label="销售属性">
        <el-select
          :placeholder="
            unSelectSaleAttrList.length > 0
              ? `还未选择${unSelectSaleAttrList.length}个销售属性`
              : '无'
          "
          style="width: 200px"
        ...
        <el-button
          ...
          @click="addSaleAttr"
          :disabled="selectSaleAttrIdAndName ? false : true"
        >
          添加销售属性
        </el-button>
        <el-table border style="margin: 10px 0" :data="spuSaleAttrList">
          ...
          <el-table-column label="销售属性值">
            <template #="{ row, $index }">
              ...
              <el-button
                v-if="!row.isInputOrButton"
                type="primary"
                icon="Plus"
                size="small"
                @click="addSaleAttrValueToInput(row)"
              ></el-button>
              <el-input
                v-else
                style="width: 50px"
...
let unSelectSaleAttrList = computed(() => {
  return saleAttrList.value.filter((item: SaleAttr) => {
    return spuSaleAttrList.value?.every(
      (item2: SaleAttr) => item.name !== item2.saleAttrName,
    )
  })
})
...
const initHasSpuData = async (spu: SPUData) => {
  // console.log('spu', spu);
  // 获取全部的品牌
  const resAllTrademarkList: TradeMarkListResponseData =
    await reqAllTrademarkList()
  trademarkList.value = resAllTrademarkList.data
  // console.log('trademarkList.value', trademarkList.value);
  // 获取某一个SPU下的全部商品图片数据
  const resSpuImageList: SpuImageListResponseData = await reqSpuImageList(
    spu.id,
  )
  ...
  // 获取全部的销售属性
  const resAllSaleAttrList: SaleAttrListResponseData =
    await reqAllSaleAttrList()
  saleAttrList.value = resAllSaleAttrList.data
  // console.log('saleAttrList.value', saleAttrList.value);
  // 获取某一个SPU下已有的销售属性
  const resSpuSaleAttrList: SaleAttrListResponseData = await reqSpuSaleAttrList(
    spu.id,
  )
...
const handleBlurToLook = (row: SaleAttr) => {
  ...
  let repeat = row.spuSaleAttrValueList.find(
    (item: SaleAttrValue) => item.saleAttrValueName === saleAttrValueName,
  )
...
const addOrUpdateSpu = async () => {
  // 整理spuFormData数据
  // 整理图片数据
  spuFormData.value.spuImageList = spuImageList.value.map(
    (item: UploadFile) => {
  ...
  if (res.code === 200) {
    ...
    emit('changeScene', {
      sceneNum: 0,
      addOrUpdate: spuFormData.value.id ? 'update' : 'add',
    })
  } else {
    ElMessage({
      type: 'error',
