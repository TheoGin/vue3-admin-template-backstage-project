    ...
    <el-card style="margin: 10px 0">
      <div v-show="scene === 0">
        ...
        <el-table border style="margin: 10px 0" :data="hasSpuList">
          ...
          <el-table-column label="操作">
            <template #="{row, $index}">
              ...
              <el-button type="info" size="small" icon="View" @click="showSkuList(row)"></el-button>
        ...
        <el-pagination v-model:current-page="currentPage" v-model:page-size="pageSize" :page-sizes="[3, 5, 7, 9, 11]"
          ...
          @current-change="getHasSpuList" @size-change="handleSizeChange" />
      </div>
      ...
      <el-dialog :title="skuList.length > 0 ? 'SKU列表' : '该SPU下没有SKU数据'" v-model="dialogVisible">
...
import type { SPUData, SkuData, SkuListResponseData } from '@/api/product/spu/type';
...
const getHasSpuList = async (pager = 1) => {
  ...
  let res: SpuListResponseData = await reqGetSpuList(currentPage.value, pageSize.value, categoryStore.c3Id)
  // console.log(res);
...
const deleteSpu = async (row: SPUData) => {
  ...
  if(res.code === 200) {
    ...
    getHasSpuList(hasSpuList.value.length > 1 ? currentPage.value : currentPage.value - 1)
  } else {
    ElMessage({
      type: 'error',
      message:'删除失败'
