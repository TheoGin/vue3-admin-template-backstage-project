        ...
        <el-table border style="margin: 10px 0;" :data="attrArr">
          ...
          <el-table-column label="操作" width="120px">
            <template #="{ row, $index }">
              ...
              <el-popconfirm :title="`你确定要删除${row.attrName}?`" icon="Delete" width="249px" @confirm="deleteAttr(row.id)">
                <template #reference>
      ...
      <div v-if="scene == 1 ? true : false">
        <el-form>
          <el-form-item label="属性名称：">
            <el-input v-model="attrFormParams.attrName" style="width: 200px" placeholder="请输入属性的名字"></el-input>
          </el-form-item>
        </el-form>
        <el-button type="primary" icon="Plus" :disabled="attrFormParams.attrName ? false : true"
          @click="handleAddAttrValue">添加属性“值”</el-button>
        <el-button type="default" @click="cancel">取消</el-button>
        <el-table border style="margin: 10px 0;" :data="attrFormParams.attrValueList">
          <el-table-column label="序号" type="index" align="center" width="80px"></el-table-column>
          <el-table-column label="属性值">
            <template #="{ row, $index }">
              <el-input :ref="(vc: any) => inputRefArr[$index] = vc" v-if="row.isInput === true ? true : false"
                @blur="handleValueNameInputBlur(row, $index)" v-model="row.valueName"></el-input>
              <div v-else @click="handleToInput(row, $index)">{{ row.valueName }}</div>
            </template>
          </el-table-column>
          <el-table-column label="操作">
            <template #="{ row, $index }">
              <el-button type="danger" icon="Delete"
                @click="attrFormParams.attrValueList.splice($index, 1)"></el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-button type="primary" @click="handleSave"
          :disabled="attrFormParams.attrValueList.length > 0 ? false : true">保存</el-button>
        <el-button type="default" @click="cancel">取消</el-button>
      </div>
    </el-card>
  </div>
